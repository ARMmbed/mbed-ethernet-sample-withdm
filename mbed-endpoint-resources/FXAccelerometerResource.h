/**
 * @file    FXAccelerometerResource.h
 * @brief   mbed CoAP Endpoint FXAccelerometer sensor resource supporting CoAP GET
 * @author  Doug Anson
 * @version 1.0
 * @see
 *
...
#ifndef __FX_ACCELEROMETER_RESOURCE_H__
#define __FX_ACCELEROMETER_RESOURCE_H__
...
/** 
 * FXAccelerometerResource class
 */
class FXAccelerometerResource : public DynamicResource
{
private:
...
public:
    ...
    @param obj_name input the FXAccelerometer Object name
    @param res_name input the FXAccelerometer Resource name
    @param observable input the resource is Observable (default: FALSE)
    */
    FXAccelerometerResource(const Logger *logger,const char *obj_name,const char *res_name,const bool observable = false) : DynamicResource(logger,obj_name,res_name,"FXAccelerometer",M2MBase::GET_ALLOWED,observable) {
        // init
        _instance = (void *)this;
    ...
    /**
    Get the value of the FXAccelerometer sensor
...
void _motion_processor(const void *args) {
    while (true) {
        if (_motion == true) {
            if (_instance != NULL) {
                // observe...
                ((FXAccelerometerResource *)_instance)->observe();
            }
            _motion = false;
        }
...
#endif // __FX_ACCELEROMETER_RESOURCE_H__
